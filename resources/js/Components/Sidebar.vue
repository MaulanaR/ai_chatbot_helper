...
const isRouteActive = (routePattern) => {
    ...
    if (routePattern === 'chatbots.*') {
        return currentUrl.startsWith('/chatbots') && !currentUrl.includes('/analytics');
    }

    if (routePattern === 'analytics.*') {
        return currentUrl === '/analytics' || currentUrl.includes('/analytics');
    }
    ...
    >
        ...
        <nav class="mt-8 px-4 space-y-2">
            ...
            <Link :href="route('analytics.index')" 
                class="flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 group"
                :class="isRouteActive('analytics.*') 
                    ? 'bg-indigo-50 text-indigo-700 shadow-sm' 
                    : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'"
            >
                <svg class="w-5 h-5 mr-3 transition-colors duration-200"
                    :class="isRouteActive('analytics.*') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500'"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
